...
- **次版の開発ソース** — [`codex/synwork-integration`](https://github.com/synchro4351/udonarium_axe/tree/codex/synwork-integration)。r1に自動再生・エフェクトのアイコン表示・カットイン見本・手札の受け渡しを追加しています。
...
- 素材なしで使えるカットイン見本「戦闘開始」「成功！」「場面転換」を追加しました。通常のシーン編集で文字・色・動きを調整できます。
- 手札のカードを右クリックまたは送信ボタンから参加者に渡せます。相手の手札へ裏向きで移り、チャット通知には参加者名だけが出ます。キャラクター別手札・相手へのドラッグ送付は未対応です。
- 本人が手札全体を公開・秘匿に切り替えられます。既定は秘匿で、再接続時も秘匿へ戻ります。公開中は相手手札の閲覧画面に表面と文章が表示されます。
- 手札を自動ソートできます。同梱トランプはスート→数字、その他は名前順。自分の表示だけに作用し、手動順にも戻せます。
- 明示的に渡されたカードに、直前の渡し主の印を表示します。クリックすると名前を確認でき、手札から出す・山札へ戻すと記録を消します。
...
- 次版のエフェクト選択欄とカットイン見本は、自動テスト・ビルドを確認済みですが、実画面の見え方・演出は目視未確認です。
- カード追加機能は自動テスト・ビルドを確認済みですが、2クライアントでの実受け渡し・公開切替の同期、画面目視、セーブ再読込は未確認です。渡し主のID・名前はカードとともに保存・同期されるため、セーブを第三者へ配布する際は留意してください。公開設定はルームセーブに残らず、自動ソート設定はブラウザ内だけに保存します。
