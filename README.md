...
- **開発ソース** — [`codex/synwork-integration`](https://github.com/synchro4351/udonarium_axe/tree/codex/synwork-integration)。現時点ではr2の固定ソースと同じコミットです。
...
- 2026-09-26に、手札一覧から引く操作とカットイン見本の演出を含む確認項目について、利用者が画面確認を完了しました。今回の「Level UP」の再調整と手札アイコンは配備後の目視待ちです。絵文字の見え方は端末のフォントに依存します。
- カードの2クライアント操作は利用者が確認済みです。追加設定のセーブ再読込、弱回線での動作、山札の通信遅延そのものの解消は未確認です。渡し主のID・名前はカードとともに保存・同期されるため、セーブを第三者へ配布する際は留意してください。公開設定はルームセーブに残らず、自動ソート設定はブラウザ内だけに保存します。
