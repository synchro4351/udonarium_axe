...
- **現在の利用版：tyoitashi r2／Axe v1.57.1** — [固定ソース `6e5668dd`](https://github.com/synchro4351/udonarium_axe/tree/6e5668dd5b71ce7071be8dcbd9dd120da25aa7b1)
- **開発ソース** — [`codex/synwork-integration`](https://github.com/synchro4351/udonarium_axe/tree/codex/synwork-integration)。r2を基点に、GMが不在参加者の手札を引き継がせる機能を追加しています。通常VPSにはまだ反映していません。
...
- GMは「全員の手札」から不在参加者の手札を伏せたまま確認し、接続中の参加者へまとめて引き継がせられます。確認操作が必要で、カードの表面は移動前に表示されません。ZIP再読込時のカード複製は別の課題です。
...
- カードの2クライアント操作は利用者が確認済みです。手札の通常のセーブ・ロードと同じタブでの再接続はユーザーが確認済みです。一方、既存卓へ保存ZIPを読み込むとカードが重複する問題が報告されています。弱回線での動作と山札の通信遅延そのものの解消も未確認です。渡し主のID・名前はカードとともに保存・同期されるため、セーブを第三者へ配布する際は留意してください。公開設定はルームセーブに残らず、自動ソート設定はブラウザ内だけに保存します。
