...
- 候補の基点で全体12,555件成功・1件skip、production build成功。図形差分の元コミットでも全体12,556件成功・1件skip、build成功。統合後の通常pre-pushは別途確認します。エフェクト選択欄・見本の演出・カード機能の目視、2クライアントの実動作、カード追加項目のセーブ再読込は未確認です。山札の通信遅延自体の解消は未確認です。
