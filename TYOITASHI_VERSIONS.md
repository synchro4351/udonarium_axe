...
- 現在の候補：[19a98d7e](https://github.com/synchro4351/udonarium_axe/commit/19a98d7e)
...
- カットイン見本の動き・文字表現を増やし、新規作成時は枠なしを既定にしました。手札の初回表示と更新通知、部屋単位の公開方式、カード編集権限、場・山札からの受け渡しも追加しました。
- 手札一覧から他人のカードをドラッグで引けるようにし、専用画面を廃止しました。カットインの余分な文字空白を修正し、見本の演出を調整しました。
- ユーザーは2026-09-26に前候補のカットイン・カード操作9項目の目視確認を完了。今回追加したドラッグで引く操作と演出調整は未確認です。結合候補の全体テストで12,677件成功・1件skip、production build成功。追加設定のセーブ再読込、弱回線での実動作、山札の通信遅延解消は未確認です。
- [r1からの差分](https://github.com/synchro4351/udonarium_axe/compare/be1fdb8a81037efab6300ad49260023552e87739...19a98d7e)
